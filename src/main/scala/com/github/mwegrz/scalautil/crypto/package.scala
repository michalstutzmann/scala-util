...
package object crypto {
  ...
  private def aes128Cmac2(key: Array[Byte], data: Array[Byte]): Array[Byte] = {
    ...
    def aes128(key: Array[Byte], data: Array[Byte]): Array[Byte] = {
      ...
      cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "AES"), new IvParameterSpec(new Array[Byte](16)))
      cipher.doFinal(data)
    }
  ...
  def aes128EcbEncrypt(key: ByteVector, data: ByteVector, padding: Padding = Padding.`NoPadding`): ByteVector = {
  ...
  def aes128EcbDecrypt(key: ByteVector, data: ByteVector, padding: Padding = Padding.`NoPadding`): ByteVector = {
    val cipher = Cipher.getInstance(s"AES/ECB/$padding", Provider)
  ...
  def aes128CbcEncrypt(key: ByteVector,
    ...
    cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key.toArray, "AES"), new IvParameterSpec(iv.toArray))
    ByteVector.view(cipher.doFinal(data.toArray, 0, data.length.toInt))
  }
  ...
  def aes128CbcDecrypt(key: ByteVector,
                       ...
                       iv: InitializationVector = InitializationVector.Zero): ByteVector = {
    val cipher = Cipher.getInstance(s"AES/CBC/$padding", Provider)
    cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key.toArray, "AES"), new IvParameterSpec(iv.toArray))
    ByteVector.view(cipher.doFinal(data.toArray, 0, data.length.toInt))
  }
