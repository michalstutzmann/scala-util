        ...
        Source(
          if (topics.nonEmpty) {
            Iterable(
              ...
              Command[() => Unit](Subscribe(topics.map { case (name, qos) => (name, qos.toControlPacketFlags) }.toSeq))
            )
          } else {
      ...
      flow
        .watchTermination() { (_, f) =>
          f.recover {
              case t: Throwable =>
                session.shutdown()
                throw log.error("Flow encountered a failure and has been restarted", t)
            }
            .map { _ =>
              session.shutdown()
