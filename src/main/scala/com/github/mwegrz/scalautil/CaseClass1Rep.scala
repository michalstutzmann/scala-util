...
object CaseClass1Rep {
  implicit def repFromCaseClass[CC <: Product, F1]: CaseClass1Rep[CC, F1] =
    macro CaseClassRepMacros.materializeCaseClass1Rep[CC, F1]
}
...
class CaseClassRepMacros(override val c: whitebox.Context) extends MacroUtils {
  ...
  def materializeCaseClass1Rep[CC <: Product: c.WeakTypeTag, F1: c.WeakTypeTag]: c.Expr[CaseClass1Rep[CC, F1]] = {
    ...
    assertCaseClass(
      CaseClass,
      s"To materialize case class representation, ${CaseClass.typeSymbol} must be a case class"
    )
    ...
    CaseClass match {
      ...
      case _ =>
        c.abort(
          c.enclosingPosition,
          "To materialize CaseClass1Rep, case class must have arity == 1"
        )
    }
  }
  ...
  private def materializeRep1(caseClassType: Type, caseAccessor: MethodSymbol) = {
    ...
    q"new _root_.com.github.mwegrz.scalautil.CaseClass1Rep[$caseClassType, $f1]($applyF, $unapplyF)"
  }
}
