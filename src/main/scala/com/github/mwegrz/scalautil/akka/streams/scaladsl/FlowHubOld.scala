...
class FlowHubOld[A, B, _](
    flow: Flow[A, B, _],
    bufferSize: Int = 256,
    perProducerBufferSize: Int = 16)(implicit actorMaterializer: ActorMaterializer)
  ...
  private val (killSwitch, source, sink) = RunnableGraph
    .fromGraph(
      GraphDSL.create(flow.viaMat(KillSwitches.single)(Keep.right), broadcastHub, mergeHub)(
        (_, _, _)) { implicit b => (i, bh, mh) =>
